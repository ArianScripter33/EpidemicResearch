...
> **Autores:** Arian Pedroza Celis · Kevin Axel Acosta Ayala · Diana Victoria Hernandez Monroy
...
La segunda capa asegura la confidencialidad en el servidor mediante un esquema de **Cifrado Híbrido (Hybrid Encryption)** que combina la agilidad de los algoritmos simétricos con la robustez de la criptografía asimétrica **RSA-2048**. Dado que el cifrado asimétrico es computacionalmente costoso para grandes volúmenes de datos, el sistema cifra los registros personales en la base de datos pecuaria usando claves simétricas eficientes, y luego encripta únicamente estas claves con la llave pública de SENASICA. La información resultante se almacena en contenedores inescrutables, jamás en texto plano. Solo la Comisión oficial (CPA/SENASICA) posee la llave privada exclusiva para descifrar la clave simétrica y 'abrir la caja fuerte' para conocer la identidad del ganadero únicamente cuando es indispensable enviar ayuda veterinaria y vacunas al predio. 
